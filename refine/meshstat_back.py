...
class Mesh:
    ...
    def get_face_aspect_ratios(self):
        return [self.calculate_triangle_aspect_ratio(face) for face in self.faces]
    ...
    def is_oriented(self):
        ...
        visited_faces = set()
        stack = [0]  # Start with the first face
        ...
        while stack:
            ...
            current_normal = face_normals[current_face_index]
            visited_faces.add(current_face_index)

            # Check adjacent faces
            for neighbor_index in self._find_adjacent_faces(current_face_index):
                if neighbor_index not in visited_faces:
                    stack.append(neighbor_index)
                    neighbor_normal = face_normals[neighbor_index]

                    # Check if normals are opposing
                    if not self._are_normals_consistent(current_normal, neighbor_normal):
                        return False
    ...
    def _build_edge_face_map(self):
        ...
        for i, face in enumerate(self.faces):
            ...
            for j in range(num_vertices):
                edge = (min(face[j], face[(j + 1) % num_vertices]), 
                        max(face[j], face[(j + 1) % num_vertices]))
                if edge not in self.edge_face_map:
                    self.edge_face_map[edge] = []
                self.edge_face_map[edge].append(i)
    ...
    def _find_adjacent_faces(self, face_index):
        ...
        for i in range(num_vertices):
            edge = (min(face[i], face[(i + 1) % num_vertices]), 
                    max(face[i], face[(i + 1) % num_vertices]))
            for adjacent_face_index in self.edge_face_map.get(edge, []):
                if adjacent_face_index != face_index:
                    adjacent_faces.add(adjacent_face_index)

        return list(adjacent_faces)

    def _are_normals_consistent(self, normal1, normal2, tolerance_angle_deg=5):
        # Convert tolerance angle to radians
        tolerance_angle_rad = np.radians(tolerance_angle_deg)

        # Calculate the cosine of the angle between the two normals
        cos_angle = np.dot(normal1, normal2) / (np.linalg.norm(normal1) * np.linalg.norm(normal2))

        # Check if the angle between normals is within the tolerance
        return np.arccos(np.clip(cos_angle, -1.0, 1.0)) <= tolerance_angle_rad   
...
if __name__ == "__main__":
    ...
    obj_path = os.path.join(crrent_path, "0000025.obj")
